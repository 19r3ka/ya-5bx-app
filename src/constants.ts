export const EXERCISE_TIMES = [120, 60, 60, 60, 360]
// exercise 6 has a mix of cardio (75 reps) and plyometric (10 reps) exercises
export const EXERCISE_5_REPS = [75, 10]
export const EXERCISE_LEVELS = [
  'D-',
  'D',
  'D+',
  'C-',
  'C',
  'C+',
  'B-',
  'B',
  'B+',
  'A-',
  'A',
  'A+',
]
export const LOCAL_STORAGE_TIMER_KEY = 'ya-5bx_timer'
export const LOCALSTORAGE_USER_KEY = 'ya-5bx_user'
export const LOCAL_STORAGE_WORKOUT_KEY = 'ya-5bx_workout'

export const SECONDS_IN_A_YEAR = 31536000 // 365 days * 24 hours * 60 minutes * 60 seconds
export const STARTER_LEVEL = 'C1_D-'
export const MINIMUM_REQUIRED_AGE = 6
export const LEVEL_ID_SEPARATOR = '_'
